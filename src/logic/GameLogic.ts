import { getLevelConfig, LevelConfig } from './GameConfig.js';
...
export class GameLogic {
    ...
    public static isMatch(a: number, b: number): boolean {
        return (a === b) || (a + b === 10);
    }
     ...
     */
    public static generateInitialBoard(level: number): Board {
    ...
    private static generateRandomPair(): [number, number] {
     ...
     */
    public static addRow(board: Board, level: number): Board {
        const config = getLevelConfig(level);
        const newCells: Cell[] = [];
        const needed = 9;
     ...
     */
    public static getRescueSuggestion(board: Board): { index1: number, index2: number } | null {
    ...
    // Helper
    private static getMatchFor(n: number): number {
    ...
    private static hasMatchOnBoard(cell: Cell, allActive: Cell[]): boolean {
